...
class FeatureDataset(BaseDataset):
    ...
    def __init__(self, opt, test):
        ...
        self.dir_F = os.path.join(opt.dataroot,opt.F_path)
        self.test = test
    ...
    def __getitem__(self, index):
        ...
        B_feature = load_feat(B_path)
        #print("B_path",B_path)
        vid_name = B_path.split(os.sep)[-1].replace('.npy','.avi')
        class_name = B_path.split(os.sep)[-2]

        F_feature = np.zeros([self.opt.frame_num,A_feature.shape[0]],dtype=np.float32)
        F_label = np.zeros([self.opt.frame_num],dtype=np.int64)
        F_path = []
        frame_index = random.sample(list(range(1,B_frame_num)),self.opt.frame_num)
        #print("frame_index is ",frame_index)
        frame_index = [str(i).zfill(6) for i in frame_index]
        #print("frame_index is ",frame_index)
        for i in range(len(frame_index)):
            path = os.path.join(self.dir_F,class_name,vid_name,frame_index[i]+'.npy')
            #print(path)
            F_path.append(path)
            F_feature[i] = load_feat(path)
            F_label[i] = B_label
